<form action="" class="md:w-1/2 space-y-5" wire:submit.prevent='editarOferta'>
    <!-- Titulo -->
    <div>
    ...
    <!-- Imagen -->
    <div>
        <x-input-label for="imagen" :value="__('Imagen actual de la oferta')" />
        <img src="{{ asset('storage/ofertas/' . $imagen)}}" alt="{{ 'Imagen Oferta ' . $titulo}}">
    </div>

    <div>
        <x-input-label for="imagen" :value="__('Cambio en la promoción gráfica de la oferta')" />
        <x-text-input id="imagen" class="block mt-1 w-full" type="file" wire:model="nueva_imagen" accept='image/*' />
